...
symbol = st.text_input("Enter Stock Symbol(e.g. AAPL, TSLA, INFY.NS):", "OLAELEC.NS")
choice = st.radio("Data Range Option:", ["Use Listing Date (Full History)", "Choose Custom Date Range"])
future_days = st.slider("Forecast next N days", 1, 30, 4)

if choice == "Choose Custom Date Range":
...
if st.button("Train & Predict"):
    with st.spinner("Downloading data and training models..."):
        ...
        if choice == "Use Listing Date (Full History)":
        ...
        st.write("Data Starts From:", listing_date)
        ...
        model = Sequential([
            LSTM(80, return_sequences=True, input_shape=(lookback, 1)),
            Dropout(0.15),
            LSTM(50),
        ...
        model.fit(X_train_lstm, y_train_lstm, epochs=30, batch_size=16, verbose=0, callbacks=[es])
        ...
        ax.plot(y_test.values, label="Actual", color='RED', linewidth=2)
        ...
        avg_range = float((df['High'] - df['Low']).tail(30).mean())

        future_preds = []
        for i in range(future_days):
            pred_open = current_close
            pred_high = pred_open + avg_range * 0.4
            pred_low = pred_open - avg_range * 0.4
            pred_return = 0.0
            pred_close = float(lr.predict([[pred_open, pred_high, pred_low, avg_volume, pred_return]])[0])
        ...
        ax_fut.plot(df.index[-100:], df["Close"].tail(100), color="blue", label="Recent Actual")
        ax_fut.plot(future_dates, future_prices, color="red", marker='o', label="Forecast")
        ax_fut.legend()
        st.pyplot(fig_fut)
